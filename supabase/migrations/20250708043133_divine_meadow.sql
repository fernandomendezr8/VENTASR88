/*
  # Units of Measure Migration

  1. New Tables
    - `units_of_measure`
      - `id` (uuid, primary key)
      - `name` (text, full name of unit)
      - `abbreviation` (text, unique abbreviation)
      - `category` (text, category type)
      - `created_at` (timestamp)

  2. Schema Changes
    - Add `unit_of_measure_id` column to `products` table
    - Add foreign key constraint to link products with units
  ...
  4. Data
    - Insert predefined units of measure for common categories
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Allow authenticated users to read units of measure" ON units_of_measure;
  DROP POLICY IF EXISTS "Allow authenticated users to manage units of measure" ON units_of_measure;
